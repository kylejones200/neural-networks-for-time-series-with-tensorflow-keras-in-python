...
class _LSTMForecaster(nn.Module):
    ...
    def __init__(self, n_features: int, hidden: int=64, output_size: int=1, n_layers: int=1, dropout: float=0.0):
        super().__init__()
        self.lstm = nn.LSTM(n_features, hidden, num_layers=n_layers, batch_first=True, dropout=dropout if n_layers > 1 else 0)
...
def _train_torch(model: nn.Module, X_train, y_train, *, epochs: int=50, batch_size: int=32, lr: float=0.001, validation_split: float=0.2, patience: int=15) -> nn.Module:
